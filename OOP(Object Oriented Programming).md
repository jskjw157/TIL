...
<br/>
+ UI 코드의 분리
+ 입력하는 방법의 코드와 출력하는 방법의 코드는 어떤 UI(콘솔, 윈도우, 웹, 모바일)를 쓰느냐에 따라 달라지게 되어 있다.

+ Has A 상속
+ 부품으로 물려받은 상속
+ 캡슐이 다른 캡슐의 객체를 부품으로 가지고 있는 상태로, 기능을 사용할 수 있는 상속
+ Composition Has A 생성할떄 필요로하는 객체를 한번에 다갖고있는 형태
+ Aggregation Has A 필요할때마다 객체를 수시로 모집해서 갖고있는 형태
+ dependancy  함수내에서 일시적으로 생성해서 사용했다가 제거하는 의존객체
